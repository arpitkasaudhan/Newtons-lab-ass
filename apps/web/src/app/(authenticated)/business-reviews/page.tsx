...
export default function TestPage() {
  ...
  const regenerateAiReply = async (reviewId: string, reviewText: string) => {
    try {
      const prompt = `Regenerate a reply for the following review: "${reviewText}"`
      const reply = await Api.Ai.chat(prompt)
      setAiReplies(prev => ({ ...prev, [reviewId]: reply }))
      enqueueSnackbar('AI reply regenerated', { variant: 'success' })
    } catch {
      enqueueSnackbar('Failed to regenerate AI reply', { variant: 'error' })
    }
  }
  ...
  <Button
    type="default"
    icon={<SendOutlined />}
    onClick={() => regenerateAiReply(review.id, review.text)}
    disabled={editingReply[review.id]}
  >
    Regenerate Reply
